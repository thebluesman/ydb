...
-- The transactionType guard belongs on a category-filtered spend total too, even though nothing here
-- branches on the sign of amount. A transfer leg can carry a spend category -- loan repayments are the
-- real case -- and it is carried on the outgoing leg alone, so the pair does not cancel and the whole
-- amount moved would be returned as spending. Ask "is this a spending figure?", not "does this mention
-- the sign of amount?". The one question that omits the guard is one ABOUT such a category (see the
-- mirror-case rule above): there the transfer rows are the answer, not contamination.
