...
7. **"Committed to main" means pushed, not just committed locally.** After committing, run `git push origin main` and confirm it succeeded (check the output — a fast-forward summary, not just "no error") before reporting the doc as landed. A local-only commit on `main` looks identical to a pushed one from `git log`, but `@historian` (and anyone else) verifying against `origin/main` will find nothing there. This has bitten this exact workflow twice (2026-07-30): once as a wrong-branch commit, once as a right-branch-but-unpushed commit — report the actual `git push` result, not just "committed to main."
