...
describe('buildNarrationSystemPrompt', () => {
  ...
  it('states that display sign is already normalized, and forbids the model adding or removing one', () => {
    const prompt = buildNarrationSystemPrompt('AED', '')
    expect(prompt).toContain('sign has also already been normalized for display')
    expect(prompt).toContain('arrives as a positive magnitude')
    expect(prompt).toMatch(/rather than by adding or removing a minus sign yourself/)
  })
