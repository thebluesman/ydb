...
- [0023 — Structured chat output is one `result` frame, shaped by the route](0023-structured-chat-output-is-one-result-frame.md) — table, card and annotated transaction list are three renderings of one NDJSON frame; the route picks the presentation from the returned rows, because a model-emitted marker would put a control channel inside the token stream. Designed, not yet implemented
- [0024 — Follow-up suggestions are route-templated and carried on their own frame](0024-follow-up-suggestions-are-route-templated.md) — a fifth NDJSON frame, filled from a closed template set rather than by a model, because a suggestion is clickable and therefore an input path; fixes the general rule that the token stream is always last
...
- [0027 — Money display sign is normalized server-side](0027-money-display-sign-normalized-server-side.md) — ADR-0020's move applied to sign: which money columns show magnitude is decided by the route from the query's own direction restriction, applied once to the row binding narration and the `result` frame share, so a spend total can no longer read positive in the sentence and negative in the table. Also corrects the frame's `money`-column set, which excluded every `/100` aggregate. Designed 2026-08-04, not yet implemented
