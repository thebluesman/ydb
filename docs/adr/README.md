...
- [0024 — Follow-up suggestions are route-templated and carried on their own frame](0024-follow-up-suggestions-are-route-templated.md) — a fifth NDJSON frame, filled from a closed template set rather than by a model, because a suggestion is clickable and therefore an input path; fixes the general rule that the token stream is always last. Designed, not yet implemented
