...
- [0025 — The Phase A verifier returns a three-label verdict on its own type, and fails open](0025-verification-pass-verdict-is-three-labels-and-fails-open.md) — concrete design for ADR-0013's Phase A: `ok` / `mismatch` / `out-of-scope` plus a route-assigned `unusable`, deliberately not `NonAnswerReason`, running on `sqlModel` at `SQL_NUM_CTX` with its own 20s timeout. The one guard in the pipeline that fails open. Designed, not yet implemented
- [0026 — Verdicts are captured in their own table, written by the route](0026-verdicts-are-captured-in-their-own-table-written-by-the-route.md) — a `ChatVerdict` table rather than columns on `ChatMessage`, because `/api/chat` has no session id and message rows are written by the client after the stream; the eval data ADR-0013's Phase B gate is read from. Designed, not yet implemented. Designed, not yet implemented
