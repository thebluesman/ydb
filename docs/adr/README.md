...
- [0023 — Structured chat output is one `result` frame, shaped by the route](0023-structured-chat-output-is-one-result-frame.md) — table, card and annotated transaction list are three renderings of one NDJSON frame; the route picks the presentation from the returned rows, because a model-emitted marker would put a control channel inside the token stream. Shipped 2026-08-03 (PR #47).
- [0024 — Follow-up suggestions are route-templated and carried on their own frame](0024-follow-up-suggestions-are-route-templated.md) — a fifth NDJSON frame, filled from a closed template set rather than by a model, because a suggestion is clickable and therefore an input path; fixes the general rule that the token stream is always last. Shipped 2026-08-03 (PR #49).
- [0025 — The Phase A verifier returns a three-label verdict on its own type, and fails open](0025-verification-pass-verdict-is-three-labels-and-fails-open.md) — concrete design for ADR-0013's Phase A: `ok` / `mismatch` / `out-of-scope` plus a route-assigned `unusable`, deliberately not `NonAnswerReason`, running on `sqlModel` at `SQL_NUM_CTX` with its own 20s timeout. The one guard in the pipeline that fails open. Shipped 2026-08-04 (PR #52).
- [0026 — Verdicts are captured in their own table, written by the route](0026-verdicts-are-captured-in-their-own-table-written-by-the-route.md) — a `ChatVerdict` table rather than columns on `ChatMessage`, because `/api/chat` has no session id and message rows are written by the client after the stream; the eval data ADR-0013's Phase B gate is read from. Shipped 2026-08-04 (PR #52).
- [0027 — Money display sign is normalized server-side](0027-money-display-sign-normalized-server-side.md) — ADR-0020's move applied to sign: which money columns show magnitude is decided by the route from the query's own direction restriction, applied once to the row binding narration and the `result` frame share, so a spend total can no longer read positive in the sentence and negative in the table. Also corrects the frame's `money`-column set, which excluded every `/100` aggregate. Shipped 2026-08-04 (PR #54); a residual gap in the groceries/travel worked examples closed same-day (PR #55).
- [0028 — The verifier is told which filters are mandatory](0028-verifier-is-told-the-mandatory-hygiene-filters.md) — the FILTER check condemned the four hygiene predicates the SQL prompt *requires* (`transactionType != 'transfer'`, `parentTransactionId IS NULL`, `reimbursementTxId IS NULL`, `status IN (…)`), which is the largest identified source of false `mismatch` in the first production verdict sample; the exemption list becomes one constant both prompt builders read, so they cannot drift. Narrow exception to ADR-0025's "claim to check, not context to trust", which is not superseded. Accepted 2026-08-09 but not actually implemented until PR #65, 2026-08-19 — see 0031's Measured note; do not trust an ADR's Status alone as proof of shipped code
- [0029 — Planning, forecast and goal questions are declined on the question](0029-planning-questions-declined-on-the-question.md) — ADR-0015's mechanism applied to a second scope class: "what should I budget", "am I on track to hit my savings goal" have no answer in a transaction ledger, and the second one returned a plausible savings figure computed from a one-transaction junk category. Accepted 2026-08-09; confirmed unimplemented by grep 2026-08-19 (same gap class as 0028's), then implemented same day — `lib/chatPlanningIntent.ts`, wired into `app/api/chat/route.ts` before SQL generation, `@qa` fixture pair per the ADR's own spec
- [0030 — `chart` is a fourth `result` presentation, and a pure re-render of the table's rows](0030-chat-result-chart-presentation.md) — amends ADR-0023's `present` enumeration: a 2-column breakdown (text + money/number → bar) or trend (date + money/number → line), 2–10 rows, classified after the existing card/transactions checks. Re-encodes the same rows the table would show and derives nothing — no deltas, no period comparison, no anomaly framing — which is how it keeps ADR-0023's "no trend field" constraint rather than violating it
- [0031 — The verifier's LABEL check judges contradiction, not descriptiveness](0031-verifier-label-check-judges-contradiction-not-descriptiveness.md) — the LABEL check was condemning correct, prompt-compliant aliases (`total`, `total_spent`) for not being self-documenting, and the anti-sign prohibition was being routed around by attacking the alias's *name* instead of the value's sign; both replaced with stated facts the model can't rephrase past, landed together with ADR-0028's exemption constant so the eval stays attributable. Accepted 2026-08-19 (PR #65): model-only precision 0.56→0.63, recall 0.90→1.00, measured once
- [0032 — Calendar boundaries are computed server-side and handed to the model as literals](0032-calendar-boundaries-computed-server-side.md) — promotes to a rule what `mostRecentMonthYm`, `quarterRange` and `weekRange` had each re-argued in a comment: where SQLite has no modifier for a boundary, or the available one has a known edge case (`weekday N`'s NO-OP, `strftime('%W')`'s year break), the server computes it and the model copies a literal. The model may derive a boundary only by prose off a supplied literal or a whole-day shift off one; a per-row boundary gets a server-authored expression copied verbatim (`weekStartExpr`). Records a pattern already shipped three times, so a fourth granularity cites it rather than re-deriving it
