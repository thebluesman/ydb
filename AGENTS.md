...
| Chat SQL scope | No balances or net worth (declined, not approximated); no planning/forecast/goal questions; no compound SELECTs (`UNION`/`UNION ALL`/`INTERSECT`/`EXCEPT`) | `docs/adr/0015-balance-scope-enforced-on-the-question.md`, `docs/adr/0029-planning-questions-declined-on-the-question.md`, `docs/adr/0011-chat-sql-no-union-compound-selects.md` |
| Chat planning-question scope | "What should I budget", "am I on track to hit my savings goal" and similar are declined on the question, pre-generation — the ledger stores transactions, not budgets, goals or forecasts | `docs/adr/0029-planning-questions-declined-on-the-question.md` |
| Chat verifier FILTER check | The four mandatory hygiene predicates are exempt, from one constant shared by the SQL prompt and the verifier prompt; the verifier still sees no worked examples (ADR-0025 stands) | `docs/adr/0028-verifier-is-told-the-mandatory-hygiene-filters.md` |
...
| Chat SQL calendar boundaries | Computed in TypeScript from `now` and handed to the model as literals (UTC, half-open); the model may only derive further boundaries by prose off a supplied literal or a whole-day shift off one, and a per-row boundary gets a server-authored expression copied verbatim | `docs/adr/0032-calendar-boundaries-computed-server-side.md` |
...
| Chat chart presentation | `chart` is a fourth `present` value on the same `result` frame, classified after `card`/`transactions` so no ADR-0023 case changes: exactly 2 columns (text + money/number = bar, date + money/number = line) and 2–`CHART_MAX_ROWS` (10) rows, everything else still `table`. A pure re-render of the rows the table would show — no deltas, no period comparison, no anomaly framing — which is how it keeps ADR-0023's "no unverifiable trend claim" constraint; amends ADR-0023's `present` enumeration in place, both stay Accepted | `docs/adr/0030-chat-result-chart-presentation.md` |
